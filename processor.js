(() => {
  ...
  var init_message_api = __esm({
    "src/transport/message.api.ts"() {
      ...
      TRANS_INIT_SUCCESS = "TRANS_INIT_SUCCESS";
  ...
  // src/transport/ultrasonic-processor.ts
  var require_ultrasonic_processor = __commonJS({
    "src/transport/ultrasonic-processor.ts"() {
      var import_ggwave = __toESM(require_ggwave());
      init_message_api();
      var UltrasonicProcessor = class extends AudioWorkletProcessor {
        ...
        handleMessage(e) {
          ...
          switch (data.type) {
            case TRANS_INIT:
              this.init(data.sampleRate).catch((e2) => {
                console.error(e2);
              });
              break;
            case TRANS_SEND_MSG: {
        ...
        async init(sampleRate = 48e3) {
          this.ggwave = await (0, import_ggwave.default)();
        ...
        process(inputs, outputs) {
          ...
          if (inputs[0][0]) {
            this.receiveBuffer.set(inputs[0][0], this.receiveBufferOffset);
            this.receiveBufferOffset += inputs[0][0].length;
